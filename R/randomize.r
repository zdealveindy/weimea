...
randomize.cwm <- function (x, permutations = 1, FUN = function (y) y, parallel = NULL, library.par = NULL, export.cl = NULL, ...)
{
  ...
  com <- attr (x, 'com')
  traits <- attr (x, 'traits')
  cwm.rand <- function (com, traits)
  {
    traits.rand <- apply (traits, 2, FUN = function (x) {pointer <- which (!is.na (x)); x[pointer] <- sample (x[pointer]); x})
    cwm (com, traits.rand) 
  }
  if (is.null (parallel))
  {
    ...
    {
      temp.result[[perm]] <- lapply (list (cwm.rand (com = com, traits = traits)), FUN = FUN)[[1]]
    }
  }  
  ...
  if (!is.null (parallel))
  {
    ...
    parallel::clusterExport (cl, varlist = c("FUN", "traits", "com", "library.par", "cwm.rand", "cwm"), envir = environment ())
    ...
    {
      lapply (list (cwm.rand (com, traits)), FUN = FUN)[[1]]
    })
