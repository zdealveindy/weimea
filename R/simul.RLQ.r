...
#' \item{\code{scenario = 1N}}{Alternative to \code{scenario = 1} with added random noise. All three matrices are linked together; normal random noise is added to tables \bold{R} and \bold{Q} (mean = 5, sd = 1), and also to matrix \bold{L} (mean = 0, sd = .2).} Note that Dray & Legendre (2008) report adding normal noise to matrix \bold{L} mean = 0 and sd = 2, but such high sd is unrealistic and is perhaps a mistype. 
